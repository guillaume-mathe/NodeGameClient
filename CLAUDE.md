...
Game state lives in `client.state` with `frame`, `timeMs`, and `players` (array with `id` field). Non-protocol fields from snapshots/deltas are merged directly into state. `timeMs` is game elapsed time (starts at 0), **not** wall-clock time.

### Source Modules

#### `src/net/Connection.js` — WebSocket Connection

Core networking class. Handles sync handshake, snapshot/delta reconciliation, action submission with auto-incrementing `clientSeq`, ack tracking, and optional auto-reconnection.

- Constructor: `new Connection(url, { token, autoReconnect, reconnectDelayMs, maxReconnectAttempts, autoAck, applyDelta })`
...
- Properties: `state`, `playerId`, `resumed`, `rtt`, `clockOffset`, `tickRateHz`, `connected`
...
#### `src/state/interpolation.js` — Lerp Utility

`lerpEntity(prev, next, alpha, fields)` — interpolates specified numeric fields, takes non-numeric fields from `next`.
...
Predicts state locally and reconciles with server authoritative state.

- Constructor: `new PredictionManager({ connection, predict, onMisprediction, capacity })`
- `predict(state, action) → newState` callback applies action locally
- `sendAction(action)` — sends with prediction and frame targeting
- `predictedState` getter, `pendingCount` getter
